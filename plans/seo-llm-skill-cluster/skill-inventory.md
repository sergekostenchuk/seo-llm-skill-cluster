...
| `ta<api-key-redacted>` vs `site-growth-orchestrator` | Both can sound like orchestration. | TASK-PLAN skill owns plan format/governance. Site-growth skill owns site-growth sequencing and specialist handoffs. |
...
| "Обнови Obsidian wiki по проекту" | `wiki-update` or `llm-wiki-maintainer` | `ta<api-key-redacted>` | Task plan can require wiki sync, but wiki skill owns vault contract. |
